...
2. npm
3. ssh（使用的putty）
4. 相应的markdown编辑软件（我用的vscode+markdown all in one）
下面进行相应的安装介绍
### git 安装
...
### npm安装
这或许就是windows广泛使用的原因了，就是基本提供了可视化的才是windows的软件，npm就是[下载](https://nodejs.org/zh-cn/)安装就完事，然后里面提供了加入到path的方法，所以点击了就可以完成。同样的在powershell里面输入：`npm` 要是没问题就完事。
### putty ssh客户端的安装
简单来说就是下载了解压然后加入路劲完事，懂吧。
### vscode markdown 插件安装
这个就超级简单了，[插件主页](https://github.com/yzhang-gh/vscode-markdown)，这就是vscode厉害的地方了，找了一帮免费大佬写插件，懂吧，我感觉vscode会一统天下。
## 正式配置hexo
这里面主要借鉴了两个人的hexo配置教程
* [教程1](https://zhuanlan.zhihu.com/p/35668237)   不说了，暂时还没搞完这里面的所有东西，只是完成了配置的这一步
* [教程2](https://www.jianshu.com/p/956c44c87fb1)  这个比较基础，我推荐这两个结合起来看。
  
总的来说就是这个两个教程配置部分主要是两个步骤：
1. 完成本地的hexo博客配置
2. 完成github上的博客设置
3. 完成ssh和github上的配置

基本上就是这三个部分，其中1、2可以替换，但是ssh和github部分的就是有点问题，由于使用的是putty的ssh-keygen进行的密钥生成，会出现一些问题，这里先不谈了。
